...
state = {
    ...
    'bsd_l': 0,    'bsd_r': 0,
    'alarm': 0,    'fault': 0,
    'drive_mode': 1,        # 0=ECO 1=NORMAL 2=SPORT, updated live from MODE:%d in UART
    'connected': False,
}
...
def parse_line(line):
    ...
    # Line 1: SPD:72.5 SOC:79.3 TRQ:75 TMP:27.1 RNG:2600 ACC:50 BRK:0 MODE:1
    # MODE:%d comes from the firmware's telemetry printf (0=ECO,1=NORMAL,2=SPORT)
    m = re.search(
        r'SPD:(\d+)\.(\d+)\s+SOC:(\d+)\.(\d+)\s+TRQ:(-?\d+)'
        r'\s+TMP:(\d+)\.(\d+)\s+RNG:(\d+)\s+ACC:(\d+)\s+BRK:(\d+)'
        r'(?:\s+MODE:(\d+))?', line)
    if m:
        g = m.groups()
        ...
        if g[10] is not None:
            # Clamp to the 3 valid modes in case of a corrupted/partial frame
            state['drive_mode'] = max(0, min(2, int(g[10])))
...
def demo_tick():
    ...
    state['connected'] = True
    # Cycle ECO -> NORMAL -> SPORT every ~15s so the mode readout is visibly live in demo mode
    state['drive_mode'] = int(_t / 15) % 3
    spd_history.append(state['speed'])
...
def draw_info(ax):
    ...
    modes    = ['ECO', 'NORMAL', 'SPORT']
    mcols    = ['#2ecc71', '#00b4d8', '#f39c12']
    mode_idx = max(0, min(2, state['drive_mode']))

    rows = [
        ('MODE',     modes[mode_idx], mcols[mode_idx]),
