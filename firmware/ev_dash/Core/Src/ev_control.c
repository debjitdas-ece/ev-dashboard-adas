...
/**
 * @brief  Automatically selects drive mode from current speed, with
 *         hysteresis so it doesn't flicker back and forth right at a
 *         threshold (e.g. speed bouncing between 99-101 km/h would
 *         otherwise toggle SPORT/NORMAL every tick).
 *
 *  SPORT:  entered above 100 km/h, held until speed drops below 95 km/h
 *  ECO:    entered below  40 km/h, held until speed rises above 45 km/h
 *  NORMAL: everything in between
 */
#define EV_AUTOMODE_SPORT_ENTER_KMH  100.0f
#define EV_AUTOMODE_SPORT_EXIT_KMH    95.0f
#define EV_AUTOMODE_ECO_ENTER_KMH     40.0f
#define EV_AUTOMODE_ECO_EXIT_KMH      45.0f

static void EV_AutoMode(EV_HandleTypeDef *ev)
{
    switch (ev->drive_mode) {
        case DRIVE_MODE_SPORT:
            if (ev->speed_kmh < EV_AUTOMODE_SPORT_EXIT_KMH) {
                ev->drive_mode = (ev->speed_kmh < EV_AUTOMODE_ECO_ENTER_KMH)
                                  ? DRIVE_MODE_ECO : DRIVE_MODE_NORMAL;
            }
            break;

        case DRIVE_MODE_ECO:
            if (ev->speed_kmh > EV_AUTOMODE_ECO_EXIT_KMH) {
                ev->drive_mode = (ev->speed_kmh > EV_AUTOMODE_SPORT_ENTER_KMH)
                                  ? DRIVE_MODE_SPORT : DRIVE_MODE_NORMAL;
            }
            break;

        default: /* DRIVE_MODE_NORMAL */
            if (ev->speed_kmh > EV_AUTOMODE_SPORT_ENTER_KMH)
                ev->drive_mode = DRIVE_MODE_SPORT;
            else if (ev->speed_kmh < EV_AUTOMODE_ECO_ENTER_KMH)
                ev->drive_mode = DRIVE_MODE_ECO;
            break;
    }
}
...
void EV_ReadADC(EV_HandleTypeDef *ev)
{
    ...
    /* Deadzone: ignore pot noise / imprecise PicSimLab slider near zero,
       so it doesn't accidentally cross EV_REGEN_THRESHOLD_PCT */
    if (ev->brake_pedal < 2.0f) {
        ev->brake_pedal = 0.0f;
    }
...
void EV_Update(EV_HandleTypeDef *ev, float dt)
{
    ...
    /* ── 1. Motor torque from accelerator ─────────────────────────────────── */
        ...
        /* Auto-select drive mode from speed. Takes effect on the NEXT tick
         * (mode_scale above was already read for this tick), same as a
         * manual "mode" shell command would. */
        EV_AutoMode(ev);
